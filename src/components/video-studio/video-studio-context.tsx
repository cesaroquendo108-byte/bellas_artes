...
import type { LocalMedia, MediaSlot, StudioAsset, VideoStudioFields, VideoStudioState } from "./types"

type Action =
  | { type: "field"; key: keyof VideoStudioFields; value: VideoStudioFields[keyof VideoStudioFields] }
...
interface VideoStudioContextValue {
  ...
  setField: <K extends keyof VideoStudioFields>(key: K, value: VideoStudioFields[K]) => void
  setLocalMedia: (slot: MediaSlot, file: File, type: LocalMedia["type"]) => void
...
function reducer(state: VideoStudioState, action: Action): VideoStudioState {
  switch (action.type) {
    ...
    case "local-media":
      return { ...state, localMedia: { ...state.localMedia, [action.slot]: action.value } }
    case "saved-asset":
      return { ...state, savedAssets: { ...state.savedAssets, [action.slot]: action.value } }
    case "feedback":
...
export function VideoStudioProvider({ operation, children }: { operation: VideoOperation; children: React.ReactNode }) {
  ...
  const setLocalMedia = useCallback((slot: MediaSlot, file: File, type: LocalMedia["type"]) => {
    const current = stateRef.current.localMedia[slot]
    if (current) URL.revokeObjectURL(current.previewUrl)
    dispatch({ type: "saved-asset", slot })
    dispatch({ type: "local-media", slot, value: { file, type, previewUrl: URL.createObjectURL(file) } })
  }, [])

  const selectSavedAsset = useCallback((slot: MediaSlot, asset?: StudioAsset) => {
    const current = stateRef.current.localMedia[slot]
    if (current) URL.revokeObjectURL(current.previewUrl)
    dispatch({ type: "local-media", slot })
...
export function useVideoStudio() {
  ...
  if (!context) throw new Error("useVideoStudio debe utilizarse dentro de VideoStudioProvider")
  return context
}
