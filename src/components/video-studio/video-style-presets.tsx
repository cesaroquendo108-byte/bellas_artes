...
const presets = [
  ...
  },
  {
    value: "editorial",
    label: "Editorial",
    gradient: "from-violet-700 to-rose-500",
  },
];
        ...
        {presets.map((preset) => {
          ...
          return (
            <button
              key={preset.value}
              type="button"
              onClick={() => setField("stylePreset", preset.value)}
              className={cn(
                "relative overflow-hidden rounded-lg border p-2 text-left transition",
                selected
            ...
            >
              <span
                className={`mb-2 block h-8 rounded-md bg-gradient-to-br ${preset.gradient}`}
              />
              <span className="text-[10px] text-slate-300">{preset.label}</span>
              {selected && (
                <Check className="absolute top-2 right-2 size-3.5 rounded-full bg-violet-500 p-0.5 text-white" />
              )}
            </button>
          );
        })}
