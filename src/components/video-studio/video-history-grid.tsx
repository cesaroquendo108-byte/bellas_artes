...
function preferredSlot(
  ...
  if (type === "video")
    return operation === "action-sync" ? "motionVideo" : "sourceVideo";
  if (type === "audio") return "audio";
  if (operation === "action-sync" || operation === "replace-character")
      ...
      <div className="grid max-h-80 grid-cols-2 gap-2 overflow-y-auto p-3 lg:max-h-none lg:flex-1 lg:grid-cols-1 xl:grid-cols-2">
        {assets.map((asset) => {
          ...
          return (
            <article
              key={asset.id}
              className="group overflow-hidden rounded-xl border border-white/[0.07] bg-white/[0.025] transition hover:border-violet-400/30"
            >
              ...
              <div className="p-2">
                ...
                <Button
                  ...
                  variant="ghost"
                  size="xs"
                  className="mt-1 h-6 w-full text-[10px] text-violet-300"
                  disabled={!slot}
