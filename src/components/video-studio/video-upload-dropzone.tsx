...
import { FileAudio, FileImage, FileVideo, FolderOpen, Trash2, UploadCloud } from "lucide-react"
...
export function VideoUploadDropzone({ slot, title, description, type, assets, maxMb = type === "video" ? 100 : 50 }: VideoUploadDropzoneProps) {
  const inputId = useId()
  const [dragging, setDragging] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const { state, setLocalMedia, clearLocalMedia, selectSavedAsset } = useVideoStudio()
  ...
  function choose(file?: File) {
    if (!file) return
    const accepts = file.type.startsWith(`${type}/`) || (type === "audio" && file.type.startsWith("audio/"))
    ...
    return (
      <div className="overflow-hidden rounded-xl border border-violet-400/20 bg-violet-500/[0.06]">
        <div className="relative aspect-video bg-black/60">
          {type === "video" && <video src={previewUrl} className="size-full object-cover" controls preload="metadata" />}
          {type === "image" && <div role="img" aria-label={name} className="size-full bg-cover bg-center" style={{ backgroundImage: `url(${previewUrl})` }} />}
          {type === "audio" && <div className="flex size-full items-center justify-center px-4"><audio src={previewUrl} controls className="w-full" /></div>}
        ...
        <div className="flex items-center gap-2 p-2.5">
          ...
          <Button type="button" variant="ghost" size="icon-sm" onClick={() => local ? clearLocalMedia(slot) : selectSavedAsset(slot)} aria-label={`Quitar ${name}`}>
  ...
  return (
    <div className="space-y-2">
      <input id={inputId} type="file" accept={acceptByType[type]} className="sr-only" onChange={(event) => { choose(event.target.files?.[0]); event.target.value = "" }} />
      <label
        htmlFor={inputId}
        className={cn(
      ...
      >
        ...
        <span className="mt-1 text-[10px] leading-4 text-slate-500">{description} · máx. {maxMb} MB</span>
      </label>
      {matchingAssets.length > 0 && (
        <Select value={null} onValueChange={(id) => selectSavedAsset(slot, matchingAssets.find((asset) => asset.id === id))}>
          ...
          <SelectContent align="start">
            {matchingAssets.map((asset) => <SelectItem key={asset.id} value={asset.id}>{asset.name}</SelectItem>)}
          </SelectContent>
        </Select>
      )}
