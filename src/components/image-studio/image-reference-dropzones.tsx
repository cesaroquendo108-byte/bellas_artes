...
export function ImageReferenceDropzones({
  ...
  function validateFiles(files: File[]) {
    ...
    if (accepted.length !== files.length) {
      setError(
        "Algunos archivos se omitieron. Usa JPEG, PNG o WEBP de hasta 30 MB.",
      );
    } else {
      setError(null);
    }
      ...
      {selectedAsset && (
        <div className="flex items-center gap-3 rounded-xl border border-violet-400/25 bg-violet-500/10 p-2.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-[11px] text-violet-200/60">
              {selectedAsset.name}
            </p>
          </div>
          <Button
            ...
            variant="ghost"
            size="icon-sm"
            onClick={onRemoveSelectedAsset}
            aria-label="Quitar referencia guardada"
          >
          ...
          return (
            <div key={category} className="min-w-0">
              ...
              >
                ...
                <span className="mt-0.5 line-clamp-1 text-[9px] text-slate-500">
                  {description}
                </span>
              </label>
            </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {(
            Object.entries(references) as Array<
              [ReferenceCategory, LocalReference[]]
            >
          ).flatMap(([category, items]) =>
            items.map((item) => (
              <div
                key={item.id}
                className="group relative size-14 shrink-0 overflow-hidden rounded-lg ring-1 ring-white/10"
              >
                <div
