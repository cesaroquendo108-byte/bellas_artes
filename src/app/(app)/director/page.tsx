...
export default async function DirectorPage({ searchParams }: { searchParams: Promise<{ template?: string; prompt?: string }> }) {
  ...
  return <DirectorHub projects={mine.projects} community={community.projects} error={mine.error ?? community.error} initialTemplate={template} initialPrompt={typeof prompt === "string" ? prompt : ""} />
}
