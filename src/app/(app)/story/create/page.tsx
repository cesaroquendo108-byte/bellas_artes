...
export default async function StoryCreatePage({ searchParams }: { searchParams: Promise<{ project?: string; template?: string; kind?: string; view?: string; prompt?: string }> }) {
  ...
  return <div className="-m-4 sm:-m-6 lg:-m-8"><StoryboardEditor project={project} assets={library.assets} template={template} kind={project?.kind ?? kind} initialView={initialView} initialPrompt={typeof params.prompt === "string" ? params.prompt : ""} /></div>
}
